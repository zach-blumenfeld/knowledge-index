...
- **`ki status` now uses `-C/--directory` (dropped the positional `[PATH]`).** It joins the other read commands (`search`/`outline`/`get`) on the `-C` convention: `ki status` (cwd) or `ki status -C <dir>`. The old `ki status <path>` positional is removed (pre-1.0). 
