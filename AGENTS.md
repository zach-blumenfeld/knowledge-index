...
| `tests/integration/`              | End-to-end tests against a real Neo4j. Auto-skip unless `KI_TEST_NEO4J_URI` / `KI_TEST_NEO4J_USER` / `KI_TEST_NEO4J_PASSWORD` are set — to bring one up, follow `references/neo4j-podman.md`. |
...
- Don't add vector indexes in v1. Embeddings are deferred; fulltext is the retrieval substrate. The `genai` plugin in the Podman setup (`references/neo4j-podman.md`) is loaded for the upgrade path but unused.
