...
| `skills/knowledge-base/SKILL.md`              | Agent-as-user routing rules (TRIGGER / PREPARE / SKIP). Ships with the published tool.                                                                                  |
...
- Don't add vector indexes in v1. Embeddings are deferred; fulltext is the retrieval substrate. The `genai` plugin in the Podman setup (`skills/knowledge-base/references/neo4j-podman.md`) is loaded for the upgrade path but unused.
...
- For agent-as-user behavior (TRIGGER / PREPARE / SKIP, auto-mode rules), `skills/knowledge-base/SKILL.md` is the contract; don't change it without updating `docs/requirements_v01_mvp.md` in the same PR.
